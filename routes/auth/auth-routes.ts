...
import { MENRequest, USER_INFO } from "../../type-definition"
import { emailValidator, encrypt, phoneNumberValidator } from "../../utilities"
...
router.post("/login", async (req: MENRequest<LOGIN_BODY>, res: Response) => {
  ...
  let token = jwt.sign({ email: identifier }, JWT_SECRET, { expiresIn: JWT_EXP_TIME })
  return res.status(200).json({ message: "login successful", token })
})

router.post("/sign-up", async (req: MENRequest<SIGN_UP_BODY>, res: Response) => {
  try {
    let { body } = req
    ...
    return res
      .status(500)
      .json({ message: "Internal server error please try again after sometime" })
  }
})
